...
public class CandleStickChart extends GridChart {
	...
	public String getAxisYGraduate(Object value) {
		...
		return String.valueOf(Math.floor(graduate * (maxPrice - minPrice)
				+ minPrice));
	}
	...
	protected void initAxisX() {
		...
		if (null != OHLCData) {
			...
			TitleX.add(String.valueOf(
					OHLCData.get(maxCandleSticksNum - 1).getDate())
					.substring(4));
		}
	...
	protected void initAxisY() {
		...
		// 改为float类型的值 float average = (int)((maxPrice - minPrice) /
		// latitudeNum)/10 * 10;
		float average = (maxPrice - minPrice) / latitudeNum;
		// �?��刻度
		for (int i = 0; i < latitudeNum; i++) {
			String value = String.valueOf(Math.floor(minPrice + i * average));
		...
		// �?���?��值
		String value = String.valueOf(Math.floor(maxPrice));
		if (value.length() < super.getAxisYMaxTitleLength()) {
			while (value.length() < super.getAxisYMaxTitleLength()) {
				value = new String(" ") + value;
			}
	...
	protected void drawCandleSticks(Canvas canvas) {
		...
		float stickWidth = ((super.getWidth() - super.getAxisMarginLeft() - super
				.getAxisMarginRight()) / maxCandleSticksNum) - 1;
		// 蜡烛棒起始绘制位置
	...
	public void addData(OHLCEntity entity) {
		if (null != entity) {
			...
			if (null == OHLCData || 0 == OHLCData.size()) {
				...
				this.minPrice = entity.getLow();
				this.maxPrice = entity.getHigh();
			}

			this.OHLCData.add(entity);

			// 最高最低延长5%
			if (this.minPrice > entity.getLow() * 0.95) {
				this.minPrice = (float) (entity.getLow() * 0.95);
			}

			if (this.maxPrice < entity.getHigh() * 1.05) {
				this.maxPrice = (float) (entity.getHigh() * 1.05);
			}
